...
def inspectFlow(point, open_doors):
    ...
    detect.run(source=imageName,
               weights="yolov5/models/trained.pt",
               view_img=False,
               device=dev,
               save_txt=True,
               project='dataPool/',
...
def exampleModule():
    ...
    while robotAPI.getState() != "StandingBy" and robotAPI.getState() != "Charging":
        event.wait(2)
    robotAPI.postFillLight(True)
    robotAPI.postCtrlPanTilt(-90, 0)
    robotAPI.postCtrlLister(1.5)
    event.wait(20)

    robotAPI.moveToPoint([203], all_open_doors)
    while robotAPI.getState() != "StandingBy" and robotAPI.getState() != "Charging":
        event.wait(2)
    robotAPI.postCtrlPanTilt(90, 0)
    robotAPI.postCtrlLister(0.1)
    event.wait(20)
    robotAPI.postFillLight(False)
